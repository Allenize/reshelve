          ...
          ...[
            ...
            ('price', Icons.attach_money_outlined, 'Price (High first)'),
                ...
                child: Container(
                  ...
                  decoration: BoxDecoration(
                    color: _sort == opt.$1 ? AppColors.accent.withOpacity(0.1) : AppColors.card,
                    borderRadius: BorderRadius.circular(12),
                    border: Border.all(
                      color: _sort == opt.$1 ? AppColors.accent.withOpacity(0.4) : AppColors.glassBorder,
                    ),
                  ),
                  child: Row(children: [
                    Icon(opt.$2, color: _sort == opt.$1 ? AppColors.accent : AppColors.muted, size: 18),
                    const SizedBox(width: 12),
                    Text(opt.$3,
                        style: TextStyle(
                            color: _sort == opt.$1 ? AppColors.accent : AppColors.textColor,
                            fontWeight: FontWeight.w600,
                            fontSize: 14)),
                    const Spacer(),
                    if (_sort == opt.$1) const Icon(Icons.check, color: AppColors.accent, size: 16),
                  ]),
                ),
              )),
        ]),
...
class _EmptyInventory extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Center(
      child: Column(mainAxisSize: MainAxisSize.min, children: [
        Icon(searching ? Icons.search_off_outlined : Icons.inventory_2_outlined,
            size: 56, color: AppColors.glassBorder.withOpacity(0.4)),
        const SizedBox(height: 12),
        Text(searching ? 'No products match' : 'No products yet',
            style: const TextStyle(fontSize: 16, fontWeight: FontWeight.w600, color: AppColors.muted)),
        const SizedBox(height: 4),
        Text(
          searching ? 'Try a different search or category' : 'Tap + to add your first product',
