              ...
              Container(
                ...
                child: Column(
                  children: recent.map((s) => ListTile(
                    ...
                    title: Text(s.items.map((i) => i.name.split(' ').first).join(', '),
                        style: const TextStyle(fontSize: 13, color: AppColors.textColor),
                        maxLines: 1, overflow: TextOverflow.ellipsis),
                    subtitle: Text(
                      s.isUtang ? 'Utang${s.customerName != null ? " — ${s.customerName}" : ""}' : '✓ Paid',
        ...
        child: Column(
          ...
          children: [
            ...
            if (outOfStock.isEmpty && lowStock.isEmpty && unpaidUtang == 0)
              ...
              ...outOfStock.map((p) => _notifItem('', '${p.name} is out of stock', 'Inventory')),
              ...lowStock.map((p) => _notifItem('', '${p.name} is running low (${p.stock} left)', 'Inventory')),
